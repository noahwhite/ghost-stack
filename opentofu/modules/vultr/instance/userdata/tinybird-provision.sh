...
if [ -z "$EXTRACT_FAILED" ]; then
    # Append analytics to any profiles already set in env.config (e.g., activitypub).
    # .env.generated is loaded last by ghost-compose.service, so it wins for COMPOSE_PROFILES.
    EXISTING_PROFILES=$(grep "^COMPOSE_PROFILES=" "$CONFIG_FILE" 2>/dev/null | cut -d= -f2 || true)
    if [ -n "$EXISTING_PROFILES" ]; then
        MERGED_PROFILES="${EXISTING_PROFILES},analytics"
    else
        MERGED_PROFILES="analytics"
    fi
...
COMPOSE_PROFILES=${MERGED_PROFILES}
EOF
else
