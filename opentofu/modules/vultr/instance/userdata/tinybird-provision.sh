...
# 3. Extract tracker token
echo "[3/4] Extracting tracker token..."
TRACKER_TOKEN=$(docker compose run --rm \
    -e TB_TOKEN="${TINYBIRD_ADMIN_TOKEN}" \
    -e TB_HOST="${TINYBIRD_API_URL:-https://api.tinybird.co}" \
    tinybird-cli token ls --format json | jq -r '.[] | select(.name=="tracker") | .token')
...
echo "[4/4] Writing .env.generated..."
